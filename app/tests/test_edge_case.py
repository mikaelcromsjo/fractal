...
from infrastructure.models import User, Group, Proposal, Comment
...
from services.round_service import RoundClosedError
...
async def create_test_data(db: AsyncSession):
    ...
    # One round
    from services.round_service import create_round, create_group, add_group_member, create_proposal
    ...
    comments = []
    for p in proposals:
        c = await create_comment(db, proposal_id=p.id, user_id=users[0].id, text=f"Comment on proposal {p.id}")
        comments.append(c)
...
async def test_vote_outside_group(db: AsyncSession):
    ...
    with pytest.raises(Exception):
        await vote_proposal(db, p.id, user_outside.id, score=5)
...
async def test_vote_after_round_closed(db: AsyncSession):
    ...
    with pytest.raises(RoundClosedError):
        await vote_proposal(db, p.id, user_id, score=5)
...
async def test_double_vote(db: AsyncSession):
    ...
    # First vote
    vote1 = await vote_proposal(db, p.id, user_id, 5)
    # Second vote should overwrite or raise
    vote2 = await vote_proposal(db, p.id, user_id, 7)

    votes = await get_votes_for_proposal_repo(db, p.id)
    assert len(votes) == 1
...
async def test_small_group_no_promotion(db: AsyncSession):
    data = await create_test_data(db)
    # Only 2 groups
    from services.round_service import create_group
...
async def test_tie_votes_representative(db: AsyncSession):
    ...
    from services.representative_service import vote_representative, get_representative_repo
...
async def test_invalid_ids(db: AsyncSession):
    ...
    with pytest.raises(Exception):
        await vote_proposal(db, 99999, 1, 5)

    with pytest.raises(Exception):
        await vote_comment(db, 99999, 1, vote=True)
    ...
    for g in groups:
        for u in users[:5]:
            ...
            for member in users[:5]:
                await vote_proposal(db, p.id, member.id, score=10)
            c = await create_comment(db, p.id, users[0].id, text=f"Comment on proposal {p.id}")
            for member in users[:5]:
                await vote_comment(db, c.id, member.id, vote=True)
